...
//void GetRandomObjectList(unsigned int amount, std::vector<std::shared_ptr<Geometry>> &object_list, std::vector<std::shared_ptr<Geometry>>) {
...
void CornellBoxScene(std::vector<std::shared_ptr<Geometry>> &object_list,
    ...
    light_source = std::make_shared<XzRect>(100, 555 - 100, 100, 555 - 100, 553, light);
    ...
    sphere = std::make_shared<Sphere>(vec3<float>(70, 60, 400), 60, metal_material);
...
void Render(const int nx, const int ny, sf::Uint8 (*pixels)[3], const std::shared_ptr<Geometry> &world,
...
int main(int argc, const char *argv[]) {
    ...
    try {
        ...
        desc.add_options()
                ...
                ("normal-mapping,n", boost::program_options::value<bool>()->default_value(normal_mapping),
                 "Produces a normal mapping of the scene.")
                ("threads", boost::program_options::value<int>(),
                 "The number of threads used. If not defined, the path tracer utilizes fully the capicity of your computer cpu capacity.")
                ("samples,s", boost::program_options::value<unsigned int>()->default_value(antialias_samples),
                 "The number of samples for each pixel.")
                ("save-to,f", boost::program_options::value<std::string>()->default_value(image_path),
                 "File is saved to.")
                ("random,r", boost::program_options::value<bool>()->default_value(random_scene),
                 "Intoduce the random scene. If false the cornell box scene is used.")
                ("num-objects,o", boost::program_options::value<unsigned int>()->default_value(number_of_objects),
                 "The number of objects in the random scene. This parameter only applies for the random scene.");
    ...
    if (random_scene) {
        //GetRandomObjectList(number_of_objects, object_list, light_list);
    } else {
    ...
    Render(nx, ny, pixels, world, lights,camera, antialias_samples, normal_mapping);
